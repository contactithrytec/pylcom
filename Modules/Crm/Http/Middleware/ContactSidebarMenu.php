    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        Menu::create('contact-sidebar-menu', function ($menu) {
            ...
            if (in_array($contact->type, ['customer', 'both'])) {
				$menu->url(action('\Modules\Crm\Http\Controllers\CrmDashboardController@barcode'), 'Scanner de code-barres', ['icon' => 'fas fa-barcode-alt',  'active' => request()->segment(1) == 'contact' && request()->segment(2) == 'barcode'])->order(5);
				
				
				$menu->url(action('\Modules\Crm\Http\Controllers\SellController@getLocationList'),
                                __('lang_v1.all_location'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'contact' && request()->segment(2) == 'locations'])->order(6);
