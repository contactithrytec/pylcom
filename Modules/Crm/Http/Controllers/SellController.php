...
class SellController extends Controller
{
	 ...
	 public function getLocationList(Request $request)
    {
        ...
        if ($request->ajax()) {
			...
			$sells->where('transactions.sub_status','location');
						$sells->addSelect('transactions.custom_field_2', 'transactions.custom_field_3');
            ...
            return Datatables::of($sells)
                ...
                ->editColumn(
                    ...
                    function ($row) {
                        ...
                        return '<span class="total-discount" data-orig-value="' . $row->$custom_field_2 . '">' . $this->transactionUtil->num_f($custom_field_2, true) . '</span>';
                    }
                )->editColumn(
                    'custom_field_3',
                    function ($row) {
                        $custom_field_3 = $row->custom_field_3.' Mois' ;

                       

                        return '<span class="total-discount" data-orig-value="' . $row->$custom_field_3 . '">' . $custom_field_3 . '</span>';
                    }
                )
                ...
                ->rawColumns(['final_total', 'action', 'total_paid','custom_field_2','custom_field_3', 'total_remaining', 'payment_status', 'invoice_no', 'discount_amount', 'tax_amount', 'total_before_tax', 'shipping_status', 'types_of_service_name', 'payment_methods', 'return_due'])
                ->make(true);
        }

        return view('crm::sell.location');
    }
