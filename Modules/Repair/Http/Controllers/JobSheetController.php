...
{   
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $job_sheets = JobSheet::with('invoices')
                    ...
                    ->select('delivery_date', 'job_sheet_no', DB::raw("CONCAT(COALESCE(technecian.surname, ''),' ',COALESCE(technecian.first_name, ''),' ',COALESCE(technecian.last_name,'')) as technecian"), DB::raw("CONCAT(COALESCE(users.surname, ''),' ',COALESCE(users.first_name, ''),' ',COALESCE(users.last_name,'')) as added_by"),
                        'contacts.name as customer', 'b.name as brand', 'rdm.name as device_model', 'serial_no', 'estimated_cost', 'rs.name as status', 'repair_job_sheets.id as id', 'repair_job_sheets.created_at as created_at', 'service_type', 'rs.color as status_color', 'bl.name as location', 'rs.is_completed_status', 'device.name as device',
                        'repair_job_sheets.hotel','repair_job_sheets.code_site','repair_job_sheets.machine_voltage','repair_job_sheets.compressor','repair_job_sheets.detector_test','repair_job_sheets.test_alarms','repair_job_sheets.entry_time', 'repair_job_sheets.exit_time','repair_job_sheets.total_travel_time_go', 'repair_job_sheets.total_travel_time_return', 'repair_job_sheets.problem_solving','repair_job_sheets.kilometers_go', 'repair_job_sheets.kilometers_return', 'repair_job_sheets.date_work', 'repair_job_sheets.machine_time', 'repair_job_sheets.compressor_time', 'repair_job_sheets.reference','repair_job_sheets.designation', 'repair_job_sheets.customer_observation', 'repair_job_sheets.supervisor_name', 'repair_job_sheets.Out_Warranty', 'repair_job_sheets.Warranty');
...

    {
        ...
        try {
            $input = $request->only('contact_id', 'service_type', 'brand_id', 'device_id', 'device_model_id', 'security_pwd', 'security_pattern', 'serial_no', 'status_id', 'delivery_date', 'estimated_cost', 'product_configuration', 'defects', 'product_condition', 'service_staff', 'location_id', 'pick_up_on_site_addr', 'comment_by_ss',
                     'gas_comp','brand_comp','voltage_comp', 'service_staff2','hotel','code_site','machine_voltage','compressor','detector_test','test_alarms','entry_time','exit_time','total_travel_time_go','total_travel_time_return','problem_solving','kilometers_go','kilometers_return','date_work','machine_time','compressor_time','reference','designation',
                'customer_observation', 'supervisor_name', 'signature', 'Out_Warranty', 'Warranty','work_hour');
    ...
    {   
        ...
        $query = JobSheet::with('customer',
                        'customer.business', 'technician','technician2',
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $input = $request->only('contact_id', 'service_type', 'brand_id', 'device_id', 'device_model_id', 'security_pwd', 'security_pattern', 'serial_no', 'status_id', 'delivery_date', 'estimated_cost', 'product_configuration', 'defects', 'product_condition', 'service_staff', 'pick_up_on_site_addr', 'comment_by_ss',
                'service_staff2','hotel','code_site','machine_voltage','compressor','detector_test','test_alarms','entry_time','exit_time','total_travel_time_go','total_travel_time_return','problem_solving','kilometers_go','kilometers_return','date_work','machine_time','compressor_time','reference','designation', 'customer_observation', '
                supervisor_name', 'signature', 'Out_Warranty', 'Warranty','work_hour ','gas_comp','brand_comp','voltage_comp');
